...
void main() {
  ...
  const cacheKey = 'weather_current_en_cairo';
  ...
  setUp(() {
    ...
    when(() => mockPreferences.getLocale()).thenReturn('en');
  ...
  group('WeatherRepositoryImpl', () {
    group('getCurrentWeather', () {
      test('returns cached data when cache hit and not forceRefresh', () async {
        ...
        expect(result.dataSource, WeatherDataSource.cache);
        verifyNever(() => mockRemoteSource.getCurrentWeather(any()));
      });

      test('fetches from server on cache miss', () async {
        when(() => mockCacheClient.get(cacheKey)).thenAnswer((_) async => null);
        when(
          () => mockRemoteSource.getCurrentWeather(testCity),
        ).thenAnswer((_) async => testModel);
        when(
          () => mockCacheClient.put(cacheKey, any()),
        ).thenAnswer((_) async {});

        final result = await sut.getCurrentWeather(testCity);

        expect(result.cityName, 'Cairo');
        expect(result.dataSource, WeatherDataSource.network);
      ...
      test('falls back to stale cache when server fails', () async {
        ...
        when(
          () => mockCacheClient.getStale(cacheKey),
        ).thenAnswer((_) async => cachedJson);

        final result = await sut.getCurrentWeather(testCity);

        expect(result.cityName, 'Cairo');
        expect(result.dataSource, WeatherDataSource.staleCache);
      ...
      test('normalizes city name for cache key (lowercase, trimmed)', () async {
        when(
          () => mockCacheClient.get('weather_current_en_cairo'),
        ).thenAnswer((_) async => cachedJson);

        await sut.getCurrentWeather('  Cairo  ');

        verify(() => mockCacheClient.get('weather_current_en_cairo')).called(1);
      });
